        ...
        <div class="card-body -mt-16">
            <div class="flex flex-col md:flex-row items-center md:items-end gap-6">
                ...
                <div class="flex-1 text-center md:text-left mb-2">
                    ...
                    <p class="text-base-content/70 font-medium">{{ $user->email }}</p>
                    <div class="flex flex-wrap justify-center md:justify-start gap-2 mt-2">
                        @if($user->is_system_admin)
                            <span class="badge badge-primary">Super Administrator</span>
                        @endif
                        <span class="badge badge-ghost">Joined {{ $user->created_at->format('F Y') }}</span>
                    </div>
                </div>

                <div class="flex-none mt-4 md:mt-0">
                    <a href="{{ route('admin.profile.edit') }}" class="btn btn-outline gap-2">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <span class="font-medium text-lg">{{ $user->is_system_admin ? 'Super Administrator' : 'User' }}</span>
                        </div>
                        <div>
                            <label class="text-xs text-base-content/60 block mb-1">Member Since</label>
                            <span class="font-medium text-lg">{{ $user->created_at->format('F d, Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
