...
<div class="flex flex-col gap-6" x-data="{ 
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold">Edit Role: {{ ucfirst($role->name) }}</h1>
    ...
    <form id="role-form" action="{{ route('admin.role-permission.update', $role) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="card-body gap-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="name" value="{{ old('name', $role->name) }}" placeholder="e.g. Manager" class="input input-bordered focus:ring-2 focus:ring-primary/20 @error('name') border-error @enderror" required @if($role->name === 'super-admin') readonly @endif />
                        @error('name') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-control w-full">
                        <label class="label font-bold text-xs uppercase tracking-widest text-base-content/60">Display Label</label>
                        <input type="text" value="{{ ucfirst($role->name) }}" class="input input-bordered opacity-50 cursor-not-allowed" disabled />
