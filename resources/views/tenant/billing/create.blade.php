...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('tenant.billing.index', $tenant->slug) }}" class="btn btn-ghost btn-sm">
                ...
                <div class="card-body p-6">
                    ...
                    <div class="form-control w-full">
                        <select id="patient_id" class="select select-bordered w-full select-lg font-medium">
                            <option value="" disabled {{ !isset($appointment) ? 'selected' : '' }}>Search or select a patient...</option>
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}" {{ (isset($appointment) && $appointment->patient_id == $patient->id) ? 'selected' : '' }}>
                                    {{ $patient->last_name }}, {{ $patient->first_name }} ({{ $patient->phone ?? 'No Phone' }})
                                </option>
                            @endforeach
                        </select>
                        @if(isset($appointment))
                            ...
                            <div class="mt-2 text-xs text-primary flex items-center gap-1">
                                ...
                                Linked to appointment on {{ $appointment->scheduled_at->format('M d, Y') }}
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="card-body p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6" id="service-grid">
                        ...
                        <div class="service-card border border-base-200 rounded-xl p-4 hover:border-primary hover:bg-primary/5 cursor-pointer transition-all group" 
                             onclick="addItem('{{ $service->id }}', '{{ $service->name }}', {{ $service->amount }})">
                            <div class="font-bold text-sm mb-1 group-hover:text-primary transition-colors">{{ $service->name }}</div>
                            <div class="text-lg font-bold">₱{{ number_format($service->amount, 2) }}</div>
                            <div class="text-[10px] uppercase tracking-wider text-base-content/50 mt-1">{{ $service->category }}</div>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div class="form-control w-full">
                        <div class="join">
                            <select id="service_search" class="select select-bordered join-item w-full">
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" data-name="{{ $service->name }}" data-price="{{ $service->amount }}">
...
<script>
    ...
    async function finalizeInvoice() {
        ...
        try {
            const response = await fetch("{{ route('tenant.billing.store', $tenant->slug) }}", {
                method: "POST",
                headers: {
