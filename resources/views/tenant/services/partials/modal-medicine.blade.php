{{-- Medicine Modal - HTML/JS only --}}
<dialog id="medicine-modal" class="modal">
    <div class="modal-box max-w-2xl">
        <h3 class="font-bold text-lg mb-4" id="medicine-modal-title">New Medicine</h3>
        <form id="medicine-form" method="POST" action="{{ route('tenant.services.store.medicine', request()->route('tenant')) }}">
            @csrf
...
function editMedicine(med) {
    ...
    document.getElementById('medicine-form').action = '{{ route("tenant.services.update.medicine", ["tenant" => request()->route("tenant"), "medicine" => "__ID__"]) }}'.replace('__ID__', med.id || med._id);
...
function resetMedicineForm() {
    ...
    document.getElementById('medicine-form').action = '{{ route("tenant.services.store.medicine", request()->route("tenant")) }}';
