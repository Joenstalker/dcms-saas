{{-- Consent Template Modal - HTML/JS only --}}
<dialog id="consent-modal" class="modal">
    <div class="modal-box max-w-2xl">
        <h3 class="font-bold text-lg mb-4" id="consent-modal-title">New Consent Template</h3>
        <form id="consent-form" method="POST" action="{{ route('tenant.services.store.consent', request()->route('tenant')) }}">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="form-control">
                    <x-wysiwyg-editor 
                        ...
                        :variables="['[[PatientName]]', '[[PatientFirstName]]', '[[PatientLastName]]', '[[GuardianName]]', '[[DentistName]]', '[[ClinicName]]', '[[Date]]']"
                    />
                </div>
                <div class="form-control">
...
function editConsentTemplate(template) {
    ...
    document.getElementById('consent-form').action = '{{ route("tenant.services.update.consent", ["tenant" => request()->route("tenant"), "template" => "__ID__"]) }}'.replace('__ID__', template.id || template._id);
...
function resetConsentForm() {
    ...
    document.getElementById('consent-form').action = '{{ route("tenant.services.store.consent", request()->route("tenant")) }}';
