            ...
            <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="card-body items-center text-center">
                        ...
                        <div class="w-full space-y-4 text-left">
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold opacity-50 tracking-wider text-secondary">Email Address</span>
                                <span class="text-sm font-medium">{{ $patient->email ?? 'N/A' }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold opacity-50 tracking-wider text-secondary">Phone Number</span>
                                <span class="text-sm font-medium">{{ $patient->phone ?? 'N/A' }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold opacity-50 tracking-wider text-secondary">Age / Gender</span>
                                <span class="text-sm font-medium">{{ $patient->dob ? $patient->dob->age . ' yrs' : 'N/A' }} / {{ ucfirst($patient->gender ?? 'N/A') }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[10px] uppercase font-bold opacity-50 tracking-wider text-secondary">Home Address</span>
                                <span class="text-sm leading-relaxed">{{ $patient->address ?? 'N/A' }}</span>
                            </div>
                        </div>

                        <div class="card-actions justify-center mt-6 w-full">
                            <a href="{{ route('tenant.patients.edit', [$tenant->slug, $patient->id]) }}" class="btn btn-secondary btn-block">
                ...
                <div class="card bg-base-100 shadow-sm border border-base-200">
                    <div class="card-body p-4">
                        ...
                        <div class="space-y-1">
                            <button class="btn btn-ghost btn-sm btn-block justify-start text-secondary">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card bg-base-100 shadow-sm border border-base-200">
                    <div class="card-body">
                        ...
                        <div class="bg-base-200/50 p-6 rounded-xl min-h-[200px] whitespace-pre-wrap text-sm leading-relaxed border border-base-200 italic opacity-80">
                            {{ $patient->medical_history ?? 'No clinical notes recorded for this patient yet.' }}
                        </div>
                    </div>
                </div>
