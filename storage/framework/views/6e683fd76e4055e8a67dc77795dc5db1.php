        ...
        <div class="overflow-x-auto">
            <table class="table table-zebra w-full">
                ...
                <tbody>
                    ...
                    <tr onclick="window.location='<?php echo e(route('tenant.patients.show', [$tenant->slug, $patient->id])); ?>'" class="hover:bg-primary/5 cursor-pointer transition-colors group">
                        <td class="pl-6 py-4">
                            <div class="flex items-center space-x-4">
                                <div class="avatar placeholder">
                                    <div class="bg-primary/10 text-primary rounded-full w-12 border-2 border-primary/20 transition-transform group-hover:scale-110">
                                        <span class="text-sm font-bold"><?php echo e(substr($patient->first_name, 0, 1)); ?><?php echo e(substr($patient->last_name, 0, 1)); ?></span>
                                    </div>
                                </div>
                                <div>
                                    <div class="font-bold text-base-content group-hover:text-primary transition-colors"><?php echo e($patient->first_name); ?> <?php echo e($patient->last_name); ?></div>
                                    <div class="text-[10px] uppercase font-bold opacity-40 tracking-widest">#PAT-<?php echo e(str_pad($patient->id, 5, '0', STR_PAD_LEFT)); ?></div>
                                </div>
                            </div>
                        </td>
                        <td>
                            ...
                            <div class="text-sm font-medium opacity-70 italic">
                                <?php echo e($lastVisitText); ?>

                            </div>
                        </td>
                        <td class="text-right pr-6">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($patient->balance > 0): ?>
                                <div class="text-lg font-black text-error">Php <?php echo e(number_format($patient->balance, 2)); ?></div>
...
<dialog id="add_patient_modal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box max-w-2xl bg-base-100 rounded-3xl shadow-2xl p-0 overflow-hidden border border-base-200">
        ...
        <form action="<?php echo e(route('tenant.patients.store', $tenant->slug)); ?>" method="POST" class="p-8 space-y-8">
            <?php echo csrf_field(); ?>
            <div class="space-y-6">
...
<script>
    async function viewPatient(id) {
        try {
            ...
            const baseUrl = "<?php echo e(route('tenant.patients.show', [$tenant->slug, ':id'])); ?>";
    ...
    async function editPatient(id) {
        try {
            // Use Laravel's route helper to generate the correct URL (subdomain or path)
            const baseUrl = "<?php echo e(route('tenant.patients.edit', [$tenant->slug, ':id'])); ?>";
            const url = baseUrl.replace(':id', id);
