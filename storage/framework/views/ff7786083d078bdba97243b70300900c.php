...
<dialog id="condition-modal" class="modal">
    <div class="modal-box max-w-2xl">
        ...
        <form id="condition-form" method="POST" action="<?php echo e(route('tenant.services.store.condition', request()->route('tenant'))); ?>">
            <?php echo csrf_field(); ?>
...
function editCondition(condition) {
    ...
    document.getElementById('condition-form').action = '<?php echo e(route("tenant.services.update.condition", ["tenant" => request()->route("tenant"), "condition" => "__ID__"])); ?>'.replace('__ID__', condition.id || condition._id);
...
function resetConditionForm() {
    ...
    document.getElementById('condition-form').action = '<?php echo e(route("tenant.services.store.condition", request()->route("tenant"))); ?>';
    document.getElementById('condition-method').value = 'POST';
    document.getElementById('condition-form').reset();
}
</script>
<?php /**PATH D:\dentistmng\dcms-saas\resources\views/tenant/services/partials/modal-condition.blade.php ENDPATH**/ ?>