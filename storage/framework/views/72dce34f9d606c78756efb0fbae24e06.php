                        ...
                        <div class="dropdown dropdown-bottom">
                            ...
                            <ul tabindex="0" class="dropdown-content z-[30] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 text-base-content mt-2 border border-base-200">
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $pricingPlans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <li>
                                        <a href="#" onclick="event.preventDefault(); updatePaymentPlan('<?php echo e($plan->id); ?>', '<?php echo e($plan->name); ?>', <?php echo e($plan->price); ?>)" class="flex justify-between items-center py-3 hover:bg-primary/5 active:bg-primary/10">
                                            <span class="font-bold"><?php echo e($plan->name); ?></span>
                                            <span class="text-xs opacity-60">₱<?php echo e(number_format($plan->price, 0)); ?></span>
                                        </a>
                                    </li>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
async function updatePaymentPlan(planId, planName, planPrice) {
    ...
    try {
        const response = await fetch("<?php echo e(route('tenant.registration.update-plan')); ?>", {
            method: 'POST',
            headers: {
...
function confirmCancelRegistration() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch("<?php echo e(route('tenant.registration.cancel')); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
async function confirmStripePayment(redirectUrl) {
    ...
    try {
        ...
        if (error) {
            ...
            try {
                const response = await fetch("<?php echo e(route('tenant.registration.confirm-payment')); ?>", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
