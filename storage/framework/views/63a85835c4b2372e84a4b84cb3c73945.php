        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="card bg-base-100 shadow-xl border border-base-200 overflow-visible">
                <div class="card-body p-6 space-y-6">
                    <form id="roleForm" action="<?php echo e($selectedRole ? route('admin.role-permission.update', $selectedRole) : '#'); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="form-control w-full">
                            ...
                            <select class="select select-bordered w-full bg-base-200/50 focus:bg-base-100 transition-all font-medium" 
                                    onchange="window.location.href='<?php echo e(route('admin.role-permission.index')); ?>?role_id=' + this.value + '<?php echo e(isset($tenant) ? '&tenant_id=' . $tenant->id : ''); ?>'">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($role->id); ?>" <?php echo e($selectedRole && $selectedRole->id == $role->id ? 'selected' : ''); ?>>
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="text" name="name" value="<?php echo e($selectedRole ? $selectedRole->name : ''); ?>" 
                        ...
                        <div class="form-control w-full">
                            ...
                            <textarea name="description" class="textarea textarea-bordered h-24 bg-base-200/50 focus:bg-base-100 transition-all font-medium resize-none" 
                                      placeholder="System Administrator with full access"><?php echo e($selectedRole && isset($selectedRole->description) ? $selectedRole->description : ''); ?></textarea>
                        </div>
                        ...
                        <div class="form-control w-full">
                            ...
                            <select name="status" class="select select-bordered w-full bg-base-200/50 focus:bg-base-100 transition-all font-medium">
                                <option value="active" <?php echo e($selectedRole && ($selectedRole->status ?? 'active') == 'active' ? 'selected' : ''); ?>>Active</option>
                                <option value="inactive" <?php echo e($selectedRole && ($selectedRole->status ?? '') == 'inactive' ? 'selected' : ''); ?>>Inactive</option>
                            </select>
                        </div>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $permissionsByModule; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $module => $permissions): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div class="collapse collapse-arrow bg-base-100 shadow-sm border border-base-200">
                        ...
                        <div class="collapse-title flex items-center justify-between pr-12 py-4">
                            ...
                            <button type="button" class="btn btn-ghost btn-xs text-primary font-bold hover:bg-primary/5" onclick="deselectCategory('<?php echo e(Str::slug($module)); ?>')">
                                DESELECT CATEGORY
                            </button>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $permissions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $permission): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label class="flex items-center gap-3 p-4 rounded-xl bg-base-200/30 hover:bg-base-200 transition-all cursor-pointer group">
                                        <input type="checkbox" name="permissions[]" value="<?php echo e($permission->name); ?>" form="roleForm"
                                               class="checkbox checkbox-primary checkbox-sm permission-checkbox-<?php echo e(Str::slug($module)); ?>"
                                               <?php echo e(in_array($permission->name, $rolePermissions) ? 'checked' : ''); ?>>
                                        <div class="flex-1">
                                            <p class="font-bold text-sm group-hover:text-primary transition-colors"><?php echo e(Str::title(str_replace(['.', '-', '_'], ' ', $permission->name))); ?></p>
...
<dialog id="new_role_modal" class="modal">
    <div class="modal-box bg-base-100 shadow-2xl rounded-2xl p-0 overflow-hidden">
        ...
        <form action="<?php echo e(route('admin.role-permission.store')); ?>" method="POST" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
